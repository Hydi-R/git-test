    ...
    layout
    {
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                field("Claimant Email"; Rec."Claimant Email")
                {
                    ApplicationArea = All;
                }
                field("Claimant Password"; Rec."Claimant Password")
                {
                    ApplicationArea = All;
                }
                ...
                field("Cheque Estate"; Rec."Cheque Estate")
                {
                    ApplicationArea = All;
                }
                field("Cheque Delivery Method"; Rec."Cheque Delivery Method")
                {
                    ApplicationArea = All;
                }
                field(Memo; Rec.Memo)
                ...
                field("Claimant Mailing Address"; Rec."Claimant Mailing Address")
                {
                    ApplicationArea = All;
                }
                field("CA Message ID"; Rec."CA Message ID")
                {
                    ApplicationArea = All;
                }
                field("CA Payment Status"; Rec."CA Payment Status")
                {
                    ApplicationArea = All;
                }
                field("Payment Import Entry No."; Rec."Payment Import Entry No.")
                {
                    ApplicationArea = All;
                    Visible = false;
                }
                field("Payment Import File Name"; Rec."Payment Import File Name")
                {
                    ApplicationArea = All;
                    Visible = false;
                }
                field("Payment Import Line No."; Rec."Payment Import Line No.")
                {
                    ApplicationArea = All;
                    Visible = false;
                }
                ...
                field("Source GL Account No."; Rec."Source GL Account No.")
                {
                    ApplicationArea = All;
                }
...
#pragma warning restore AA0219
            ...
            group(Integrations)
            {
                action(FetchFromSharepoint)
                {
                    ...
                    trigger OnAction()
                    var
                        ExtfileStorage: Codeunit "External File Storage";
                    begin
                        CASharepointManagement.ProcessSharePointFiles(); // Deloitte AK 06092026
                    end;
                }
                action("SharePoint Logs")
                {
                    ApplicationArea = All;
                    Image = View;
                    RunObject = page CASharePointLogs;
                    RunPageLink = "Entry No." = field("Payment Import Entry No.");
                }
                //RH 050926--
                action("Download ISO File")
                {
                    ApplicationArea = All;
                    Image = XMLFile;
                    Caption = 'Download ISO File';
                    trigger OnAction()
                    var
                    ...
                    begin
                        ...
                        SelectedFilter := SelectionFilterManagement.GetSelectionFilter(RecRef, GenJnlLine.FieldNo("Line No."));
                        // if GenJnlLine.FindSet() then
                        //     repeat
                        //         if GenJnlLine."Document Type" <> GenJnlLine."Document Type"::Payment then
                        //             Error('Document Type should be Payment in the Selected Line No. %1', GenJnlLine."Line No.");
                        //         if (GenJnlLine."Payment Method Code" <> 'WIRE') then
                        //             Error('Selected Line No. - %1 should have Payment Method code as "WIRE".', GenJnlLine."Line No.");
                        //         if (GenJnlLine."Account Type" <> GenJnlLine."Account Type"::Vendor) then
                        //             Error('Selected Line No. - %1 should have Account Type as "Vendor".', GenJnlLine."Line No.");
                        //     until GenJnlLine.Next() = 0;

                        // GenJnlLine.SetRange("Payment Method Code", 'WIRE');
                        // GenJnlLine.SetRange("Account Type", GenJnlLine."Account Type"::Vendor);
                        //RH 070926++
                        CAISOPaymentFileManagement.GeneratePaymentISOFile(GenJnlLine, true);
                        //RH 070926--
                ...
                action(SendToSFTP)
                {
                    ...
                    begin
                        ...
                        RecRef.GetTable(GenJnlLine);
                        SelectedFilter := SelectionFilterManagement.GetSelectionFilter(RecRef, GenJnlLine.FieldNo("Line No."));
                        CAISOPaymentFileManagement.GeneratePaymentISOFile(GenJnlLine, false);
                    end;
                }
                //RH 050926--
                action("SFTP Logs")
                {
                    ApplicationArea = All;
                    Image = View;
                    RunObject = page CASFTPLogs;
                    RunPageLink = "Msg ID" = field("CA Message ID");
                }
            }
            group(Approval)
            {
        ...
        area(Promoted)
        {
            ...
            group(Category_New)
            {
                ...
                actionref(FetchFromOneDrive_Promoted; FetchFromSharepoint)
                {
                }
                actionref("SharePoint Logs Promoted"; "SharePoint Logs")
                {
                }
                actionref("Download ISO File_Promoted"; "Download ISO File")
                {
                }
                actionref("SendToSFTP_Promoted"; SendToSFTP)
                {
                }
                actionref("SFTP Logs_Promoted"; "SFTP Logs")
                {
                }
            }
        }
    ...
    begin
        ...
        GenJnlBatch.SetRange("Claimant Batch", true);
    ...
    var
        // AzureBlobStorageMgt: Codeunit CAAzureBlobStorageManagement;
        CAISOPaymentFileManagement: Codeunit CAISOPaymentFileManagement;
        CASharepointManagement: Codeunit CASharepointManagement;
        CSVBuffer: Record "CSV Buffer" temporary;
        MissingProjectPaymentMethodErr: Label 'Select a project and payment method before importing.';
}